...
import edu.example.springlab.domain.LottoVO;
...
public class LottoController {
    ...
    public String handle(@ModelAttribute("choice") LottoVO vo, int lottoNum, HttpSession s){
        ...
        /* 결과 넘겨주기 */
        if (count >= 10 ){
            vo.setMessage("로또 응모는 10번 까지만 가능합니다<br>브라우저를 재기동한 후에 응모한 후에 응모해 주세요");
        }else {
            if (lottoNum == randNum) {
                vo.setMessage("추카추카");
